...
def validate_input(job_input):
    ...
    # Validate optional reference images. They use the same base64/Data URI
    # format as the image value in the existing `images` input.
    for image_name in ("person_image", "style_image"):
        image_value = job_input.get(image_name)
        if image_value is not None and (
            not isinstance(image_value, str) or not image_value.strip()
        ):
            return (
                None,
                f"'{image_name}' must be a non-empty base64 or Data URI string",
            )
    ...
    return {
        "workflow": workflow,
        "images": images,
        "person_image": job_input.get("person_image"),
        "style_image": job_input.get("style_image"),
        "comfy_org_api_key": comfy_org_api_key,
    }, None


def prepare_reference_images(workflow, person_image=None, style_image=None):
    """Prepare reference image uploads and bind them to the workflow nodes."""
    reference_images = []
    bindings = (
        (person_image, "person_image.png", "RUNPOD_PERSON_IMAGE"),
        (style_image, "style_image.png", "RUNPOD_STYLE_IMAGE"),
    )

    for image_data, filename, node_title in bindings:
        if image_data is None:
            continue

        reference_images.append({"name": filename, "image": image_data})
        ...
        for node in workflow.values():
            ...
            if (
                ...
                and metadata.get("title") == node_title
...
def handler(job):
    ...
    input_images = list(validated_data.get("images") or [])
    input_images.extend(
        prepare_reference_images(
            workflow,
            validated_data.get("person_image"),
            validated_data.get("style_image"),
        )
    )
